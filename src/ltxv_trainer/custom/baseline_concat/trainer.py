...
class CustomTemporalConcatReferenceVideoTrainingStrategy(TrainingStrategy):
    ...
    def prepare_batch(self, batch: CustomDL3DV10KDatasetBatch, timestep_sampler: TimestepSampler):
        return self.prepare_batch_baseline_concat(batch, timestep_sampler)

    def prepare_batch_baseline_concat(self, batch: CustomDL3DV10KDatasetBatch, timestep_sampler: TimestepSampler) -> TrainingBatch:
        target = encode_video(self._vae, batch['target_pixels'])
        reference = encode_video(self._vae, batch['reference_pixels'])

        # !!!
        b, t, c, h, w = batch['condition_pixels'].shape
        condition = encode_video(self._vae, rearrange(batch['condition_pixels'], 'b t c h w -> (b t) 1 c h w'))

        target_latents: Float[torch.Tensor, 'b fhw c'] = target['latents']
        reference_latents: Float[torch.Tensor, 'b fhw c'] = reference['latents']
        condition_latents: Float[torch.Tensor, 'b fhw c'] = rearrange(condition['latents'], '(b t) n c -> b (t n) c', t=t)
        reference_latents = torch.cat([reference_latents, condition_latents], dim=1)
        condition_latent_frames = reference['num_frames'] + t
        ...
        conditioning_mask = torch.cat([reference_mask, target_conditioning_mask], dim=1)
        ...
        ref_video_coords = prepare_video_coordinates(
            num_frames=condition_latent_frames,
            height=latent_height,
            width=latent_width,
            batch_size=batch_size,
...
class CustomTrainer(LtxvTrainer):
    ...
    def _init_dataloader(self) -> None:
        ...
        if self._dataset is None:
            ...
            self._dataset = CustomDL3DV10KDataset(self._config.data.preprocessed_data_root, num_frames=25, num_ref_frames=4, discrete_condition_indices=True)

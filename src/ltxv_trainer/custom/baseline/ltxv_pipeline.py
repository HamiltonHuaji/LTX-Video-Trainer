...
class LTXTemporalConcatReferenceConditionPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    ...
    def __call__(
        ...
        reference_video: Tuple[torch.Tensor, torch.Tensor] | None = None,
        ...
        if reference_video is not None:
            ...
            _cond_latents = retrieve_latents(self.vae.encode(_condition_tensor), generator=generator)
            _cond_latents = self._normalize_latents(
                _cond_latents, self.vae.latents_mean, self.vae.latents_std
            ).to(device, dtype=torch.float32)
            ...
            _cond_video_coords = self._prepare_video_ids(
                batch_size * num_videos_per_prompt,
                _cond_latent_frames,
                _cond_latent_height,
                _cond_latent_width,
            ...
            _cond_video_coords = self._scale_video_ids(
                _cond_video_coords,
                scale_factor=self.vae_spatial_compression_ratio,
                scale_factor_t=self.vae_temporal_compression_ratio,
                frame_index=0,  # Reference video starts at frame 0
            ...
            latents = torch.cat([_cond_latents, reference_latents, latents], dim=1)

            # Update video coordinates: [reference_coords, existing_coords]
            reference_video_coords = reference_video_coords.float()
            _cond_video_coords = _cond_video_coords.float()
            video_coords = torch.cat([_cond_video_coords, reference_video_coords, video_coords], dim=2)
            ...
            if conditioning_mask is not None:
                ...
                conditioning_mask = torch.cat([_cond_conditioning_mask, reference_conditioning_mask, conditioning_mask], dim=1)
            else:
                ...
                conditioning_mask = torch.cat([_cond_conditioning_mask, conditioning_mask, target_conditioning_mask], dim=1)
        ...
        if reference_video is not None and output_reference_comparison:
            ...
            reference_latents_out = latents[:, _cond_num_latents:reference_num_latents+_cond_num_latents]

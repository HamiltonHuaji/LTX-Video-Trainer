...
class LTXTemporalConcatReferenceConditionPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    r"""
    ...
    def __call__(
        self,
        conditions: Union[LTXVideoCondition, List[LTXVideoCondition]] | None = None,
        image: Union[PipelineImageInput, List[PipelineImageInput]] | None = None,
        ...
        reference_video: Tuple[torch.Tensor, torch.Tensor] | None | None = None,
        output_reference_comparison: bool = False,
        prompt_embeds: Optional[torch.Tensor] | None = None,
        prompt_attention_mask: Optional[torch.Tensor] | None = None,
        negative_prompt_embeds: Optional[torch.Tensor] | None = None,
        negative_prompt_attention_mask: Optional[torch.Tensor] | None = None,
        decode_timestep: Union[float, List[float]] = 0.0,
        decode_noise_scale: Optional[Union[float, List[float]]] | None = None,
        output_type: Optional[str] = "pil",
        return_dict: bool = True,
        attention_kwargs: Optional[Dict[str, Any]] | None = None,
        callback_on_step_end: Optional[Callable[[int, int, Dict], None]] | None = None,
        ...
        self.check_inputs(
            ...
            reference_video=reference_video[1] if isinstance(reference_video, tuple) else reference_video,
        )
        ...
        (
            ...
            num_videos_per_prompt=num_videos_per_prompt or 1,
            prompt_embeds=prompt_embeds,
            negative_prompt_embeds=negative_prompt_embeds,
            prompt_attention_mask=prompt_attention_mask,
            negative_prompt_attention_mask=negative_prompt_attention_mask,
        ...
        if self.do_classifier_free_guidance:
            ...
            # we do not apply classifier free guidance on prompts
        ...
        if is_conditioning_image_or_video:
            for condition_image, condition_video, condition_frame_index, condition_strength in zip(image, video, frame_index, strength, strict=False):
        ...
        if reference_video is not None:
            ...
            cond_frames, ref_frames = reference_video  # [F, C, H, W]
            ...
            cond_frames = resize(cond_frames, [resize_height, resize_width], antialias=True)

            # Center crop to target dimensions
            ref_frames = center_crop(ref_frames, [height, width])
            cond_frames = center_crop(cond_frames, [height, width])
            ...
            _condition_tensor = cond_frames.unsqueeze(0).permute(0, 2, 1, 3, 4) * 2. - 1.
            ...
            _cond_num_frames_input = _condition_tensor.size(2)
            # _cond_num_frames_output = self.trim_conditioning_sequence(0, _cond_num_frames_input, num_frames)
            # _cond_num_frames_output = torch.randperm(_cond_num_frames_input)[0:4].tolist()
            _cond_num_frames_output = [int(i) for i in torch.linspace(0, _cond_num_frames_input - 1, 8).tolist()]
            _condition_tensor = _condition_tensor[:, :, _cond_num_frames_output]
            _condition_tensor = _condition_tensor.to(device, dtype=vae_dtype)
            _b, _c, _t, _h, _w = _condition_tensor.shape
            ...
            _condition_tensor = _condition_tensor.repeat(batch_size * num_videos_per_prompt, 1, 1, 1, 1)
            ...
            _cond_latents = retrieve_latents(self.vae.encode(rearrange(_condition_tensor, 'b c t h w -> (b t) c 1 h w')), generator=generator)
            _cond_latents = rearrange(_cond_latents, '(b t) c n h w -> b c (t n) h w', t=_t)
            _cond_latents = self._normalize_latents(_cond_latents, self.vae.latents_mean, self.vae.latents_std).to(device, dtype=torch.float32)
            if dinov3 is not None:
                with torch.no_grad(), torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=False):
                    # image_encoder_hidden_states = self._dinov3.get_intermediate_layers(dinov3_transform(flatten_condition_pixels), n=1, reshape=True)
                    # image_encoder_token_range = (reference_num_frames, reference_num_frames + condition_num_frames)
                    from ..base import dinov3_feature
                    image_encoder_hidden_states = dinov3_feature(dinov3, rearrange(_condition_tensor.float(), 'b c t h w -> b t c h w') * .5 + .5)
            else:
            ...
            _cond_latent_frames = _cond_latents.size(2)
            _cond_latent_height = _cond_latents.size(3)
            _cond_latent_width = _cond_latents.size(4)

            reference_video_coords = self._prepare_video_ids(
                batch_size * num_videos_per_prompt,
                ref_latent_frames + _cond_latent_frames,
                ref_latent_height,
                ref_latent_width,
                patch_size_t=self.transformer_temporal_patch_size,
                patch_size=self.transformer_spatial_patch_size,
            ...
            _cond_latents = self._pack_latents(
                _cond_latents,
                self.transformer_spatial_patch_size,
                self.transformer_temporal_patch_size,
            )
            _cond_num_latents = _cond_latents.size(1)

            # Concatenate reference latents at the beginning: [reference_latents, frame_conditions, target_latents]
            latents = torch.cat([reference_latents, _cond_latents, latents], dim=1)
            if dinov3 is not None:
                image_encoder_token_range = (reference_num_latents, reference_num_latents + _cond_num_latents)
            else:
            ...
            if conditioning_mask is not None:
                ...
                _cond_conditioning_mask = torch.ones(
                    (batch_size * num_videos_per_prompt, _cond_num_latents), device=device, dtype=torch.float32
                )
                conditioning_mask = torch.cat([reference_conditioning_mask, _cond_conditioning_mask, conditioning_mask], dim=1)
            else:
                ...
                target_conditioning_mask = torch.zeros(
                    (batch_size * num_videos_per_prompt, latents.size(1) - reference_num_latents - _cond_num_latents),
                    device=device,
                    dtype=torch.float32,
                )
                conditioning_mask = torch.cat([conditioning_mask, _cond_conditioning_mask, target_conditioning_mask], dim=1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if image_encoder_hidden_states is not None:
                    image_encoder_hidden_states_input = torch.cat([torch.zeros_like(image_encoder_hidden_states), image_encoder_hidden_states]) if self.do_classifier_free_guidance else image_encoder_hidden_states
                else:
                    image_encoder_hidden_states_input = None
                ...
                noise_pred = self.transformer(
                    ...
                    return_dict=False,
                    image_encoder_hidden_states=image_encoder_hidden_states_input,
        ...
        if reference_video is not None and output_reference_comparison:
            ...
            remaining_latents = latents[:, reference_num_latents+_cond_num_latents:]
            ...
            if reference_video is not None:
                ...
                latents = latents[:, reference_num_latents+_cond_num_latents:]
